...
func (s *SellImpl) Sell(p *model.Params) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		// 更新库存
		restockUnitPrice, err := s.updateStock(tx, p.StockID, p.Quantity)
		if err != nil {
		...
		sellOrder.CalProfit(restockUnitPrice)
...
func (s *SellImpl) Update(id uint, p *model.Params) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		// 更新库存
		var restockUnitPrice float64
		if oldSO.StockID != newSO.StockID {
			// 库存变更，同时更新新旧库存
			if _, err := s.updateStock(tx, oldSO.StockID, -oldSO.Quantity); err != nil {
				return errors.WithMessagef(err, "更新出货订单过程中，更新原库存出错：%+v", oldSO)
			}
			var err error
			if restockUnitPrice, err = s.updateStock(tx, newSO.StockID, newSO.Quantity); err != nil {
				return errors.WithMessagef(err, "更新出货订单过程中，更新新库存出错：%+v", newSO)
			}
		} else if oldSO.Quantity != newSO.Quantity {
			var err error
			if restockUnitPrice, err = s.updateStock(tx, newSO.StockID, newSO.Quantity-oldSO.Quantity); err != nil {
		...
		newSO.CalProfit(restockUnitPrice)
...
func (s *SellImpl) Delete(id uint) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		// 更新库存
		if _, err := s.updateStock(tx, sellOrder.StockID, -sellOrder.Quantity); err != nil {
...
// updateStock 更新关联的库存
func (s SellImpl) updateStock(tx *gorm.DB, stockID uint, quantity float64) (float64, error) {
	var stock *stockModel.Stock
	if err := tx.First(&stock, stockID).Error; err != nil {
		return 0, errors.Wrapf(err, "更新关联库存过程中，查询库存出错：%d", stockID)
	}
	...
	if err := tx.Save(stock).Error; err != nil {
		return 0, errors.Wrapf(err, "更新关联库存过程中，更新库存出错：%+v", stock)
	}
	return stock.UnitPrice, nil
}
